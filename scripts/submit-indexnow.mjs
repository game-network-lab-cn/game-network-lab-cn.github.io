...
const origin = configuredOrigin || 'https://game-network-lab-cn.github.io';
const base = configuredOrigin ? '' : '/game-network-lab';
...
if (!sitemapResponse?.ok) {
  throw new Error(`Unable to read deployed sitemap: ${sitemapResponse?.status || 'network error'}`);
}

const sitemap = await sitemapResponse.text();
const urlList = [...sitemap.matchAll(/<loc>(.*?)<\/loc>/g)].map((match) => match[1]);
if (urlList.length === 0) throw new Error('No indexed URLs found in sitemap.');
...
const response = await fetch('https://api.indexnow.org/indexnow', {
  ...
  body: JSON.stringify({
    ...
    key,
    keyLocation: `${origin}${base}/${key}.txt`,
    urlList,
  }),
});
